...
- 🧾 **启动与运行文本日志** — 额外生成 `archiveflow.log` 文本日志，记录启动步骤、关键用户操作、后端调用成功/失败与错误信息，便于安装后卡启动时让用户直接导出日志
...
- 🪟 **右下角日志入口** — 全局浮动按钮可直接打开日志目录，在启动页卡住时也能访问
...
- 文本日志目录：Windows 下默认位于 `%APPDATA%\com.archiveflow.app\logs\`
- 当前主日志文件：`archiveflow.log`
- 启动页和主界面右下角都提供“日志”浮动按钮，可直接打开日志目录
- 日志格式示例：`[2026-06-15 22:10:31] [INFO] [BOOT] 启动步骤 2/6 完成: 建立数据库连接 (41ms)`
- `tauri dev` 已在 Vite watcher 中排除 `src-tauri/target/**`，避免 Windows 下监听 Rust 产物 `.exe` 时触发 `EBUSY`
